...
void* threadRender (void *start) {
    ...
    /* for every pixel */
    for( int px = *(int*)start; px<width; px += nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    int startIDs[nthreads];
    for (int i = 0; i < nthreads; i++) {
        startIDs[i] = i;
        if (pthread_create(&threads[i], NULL, threadRender, &startIDs[i])) {
    ...
    for (int i = 0; i < nthreads; i++)
        if (pthread_join(threads[i], NULL)) {
            fprintf(stderr, "ERROR: Thread could not be freed!");
            exit(1);
        }
